...
tar -xf $MODPATH/common/COOLBOOT -C $MODPATH 2>/dev/null
chmod +x ${MODPATH}/binary/*
[ "$($MODPATH/binary/cool32 -v)" ] && bits=32
[ "$($MODPATH/binary/cool64 -v)" ] && bits=64
mkdir -p $MODPATH/system/bin
cp -af $MODPATH/binary/cool$bits $MODPATH/system/bin/coolboot
rm -r $MODPATH/binary
### Install
#Template by @Jai_08
#Based on MMT-EX
filep=$MODPATH/common/cool_modules/bootanimation.zip
filep2=$MODPATH/common/cool_modules/bootaudio.mp3
path1=/system/media
path2=/system/product/media
maininstall()
{
if [ -f "$path1/bootanimation.zip" ]; then
mkdir -p $MODPATH$path1
cp -f $filep $MODPATH$path1
cp -f $filep2 $MODPATH$path1
elif [ -f "$path2/bootanimation.zip" ]; then
mkdir -p $MODPATH$path2
cp -f $filep $MODPATH$path2
cp -f $filep2 $MODPATH$path2
else
...
dark()
{
if [ -f "$path1/bootanimation-dark.zip" ]; then
cp -f $filep $MODPATH/$path1/bootanimation-dark.zip
elif [ -f "$path2/bootanimation-dark.zip" ]; then
cp -f $filep $MODPATH/$path2/bootanimation-dark.zip
fi
}
### Install
